...
public class SAPCollector extends DBCollector {
    ...
    private static final String[] SAP_SYS_VIEW = {
    	"m_host_agent_metrics", "m_caches", "m_disk_usage",
        "m_garbage_collection_statistics", 
        "m_host_resource_utilization", "m_data_volumes"
    };

    private static final String[] SAP_SYS_LOCAL_VIEW = {
        "m_table_statistics", "m_rs_indexes"
    };
    private static final String[] SAP_SYS_VIEW_GLOBAL = {"m_host_agent_metrics", "m_caches", "m_disk_usage",
                                                        "m_garbage_collection_statistics", "m_host_resource_utilization", 
                                                        "m_data_volumes"};
    private static final String[] SAP_SYS_VIEW_GLOBAL_KEY = {"instance_id", "cache_id", "usage_type", "store_type"
                                                        , "host", "volume_id"};
...
//            Catalog.setSeparator(conn);
            ...
            while (out.next()) {
                dbParameters.put("(" + out.getString("FILE_NAME") + ","
              + out.getString("LAYER_NAME") + ","
              + out.getString("TENANT_NAME") + ","
              + out.getString("HOST") + "," 
              + out.getString("SECTION") + ","
              + out.getString("KEY") + ")", out.getString("VALUE"));
            }
